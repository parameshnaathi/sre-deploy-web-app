...
def generate_deployment_plan(client: str, version: str, variant: str):
    ...
    plan.append({"heading": "Full Indexing",
                 ...
                 "validation": f"https://data-prd-{variant}-{client}.aws.mdx.med/index_status"})

    plan.append({"heading": "Referencing",
                 "commands": [f"dradis index reference for {client} production_{variant}"],
                 "validation": f"https://solr-master-prd-{variant}-{client}.aws.mdx.med:8443/solr/reference/select?fq=type%3A%22PlatformSearch%3A%3ADocuments%3A%3AReferenceBatchMeta%22&q=*%3A*&sort=batch_created_at_ds+desc"})

    plan.append({"heading": "Billing Codes",
                 "commands": [f"dradis index billing_codes for {client} production_{variant}"],
                 "validation": f"https://solr-cost-prd-{variant}-{client}.aws.mdx.med:8443/solr/cost/select?fq=type%3A%22PlatformSearch%3A%3ADocuments%3A%3ABillingCode%22&q=*%3A*&sort=batch_created_at_ds+desc"})

    plan.append({"heading": "Solr: Replication",
                 "commands": [f"dradis cap solr:replicate for etl {client} production_{variant} debug"],
                 "validation": f"https://data-prd-{variant}-{client}.aws.mdx.med/replication_status"})
